...
# 2. Build and push multi-arch Debian image
docker buildx build --platform linux/amd64,linux/arm64 --build-arg BASE_IMAGE=${base} -t ${debian} --push -f ${BASE_DIR}/Dockerfile.debian ${BASE_DIR}
if [ $? -ne 0 ]; then
